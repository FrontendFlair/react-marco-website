export const MenuLinks = [
    {
        title: "Home",
        url: "./"
    },
    {
        title: "About",
        url: "./about"
    },
    {
        title: "Menu",
        url: "./menu"
    },
    {
        title: "Gallery",
        url: "./gallery"
    },
    {
        title: "Contact",
        url: "./contact"
    }
]

const foodItems = [
    {
        title: "Pancakes",
        price: 5.99,
        description: "Fluffy pancakes served with maple syrup and butter.",
        category: "Breakfast"
    },
    {
        title: "Eggs Benedict",
        price: 8.49,
        description: "Classic Eggs Benedict with hollandaise sauce, ham, and poached eggs on an English muffin.",
        category: "Breakfast"
    },
    {
        title: "Avocado Toast",
        price: 6.99,
        description: "Sliced avocado on toasted artisan bread with cherry tomatoes and arugula.",
        category: "Breakfast"
    },
    {
        title: "Caesar Salad",
        price: 10.99,
        description: "Crisp romaine lettuce, croutons, Parmesan cheese, and Caesar dressing.",
        category: "Lunch"
    },
    {
        title: "Caprese Panini",
        price: 9.49,
        description: "Fresh mozzarella, tomatoes, basil, and balsamic glaze on grilled ciabatta bread.",
        category: "Lunch"
    },
    {
        title: "Margherita Pizza",
        price: 12.99,
        description: "Classic Margherita pizza with tomato sauce, fresh mozzarella, basil, and olive oil.",
        category: "Lunch"
    },
    {
        title: "Grilled Salmon",
        price: 16.99,
        description: "Grilled salmon fillet served with roasted vegetables and lemon butter sauce.",
        category: "Dinner"
    },
    {
        title: "Filet Mignon",
        price: 24.99,
        description: "Tender filet mignon steak cooked to perfection, served with mashed potatoes and asparagus.",
        category: "Dinner"
    },
    {
        title: "Chicken Alfredo",
        price: 14.99,
        description: "Creamy Alfredo sauce tossed with fettuccine pasta and grilled chicken.",
        category: "Dinner"
    },
    {
        title: "Veggie Burger",
        price: 7.99,
        description: "Grilled veggie patty topped with lettuce, tomato, onion, and pickles on a toasted bun.",
        category: "Budget Meal"
    },
    {
        title: "Spaghetti Aglio e Olio",
        price: 9.49,
        description: "Spaghetti tossed with olive oil, garlic, chili flakes, and parsley.",
        category: "Budget Meal"
    },
    {
        title: "Soup and Salad Combo",
        price: 8.99,
        description: "Choice of soup with a side salad.",
        category: "Budget Meal"
    },
    {
        title: "Buffet Brunch",
        price: 19.99,
        description: "All-you-can-eat brunch buffet featuring a variety of breakfast and lunch items.",
        category: "Buffet"
    },
    {
        title: "Seafood Buffet",
        price: 29.99,
        description: "Extensive buffet featuring a selection of fresh seafood dishes.",
        category: "Buffet"
    }
];

export default foodItems;

export const foodgallery = [
    {
        category: "Breakfast",
        url: "https://images.pexels.com/photos/376464/pexels-photo-376464.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Breakfast",
        url: "https://images.pexels.com/photos/70497/pexels-photo-70497.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Breakfast",
        url: "https://images.pexels.com/photos/1099680/pexels-photo-1099680.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Lunch",
        url: "https://images.pexels.com/photos/1640772/pexels-photo-1640772.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Lunch",
        url: "https://images.pexels.com/photos/1199957/pexels-photo-1199957.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Lunch",
        url: "https://images.pexels.com/photos/262959/pexels-photo-262959.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Dinner",
        url: "https://images.pexels.com/photos/769289/pexels-photo-769289.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Dinner",
        url: "https://images.pexels.com/photos/2097090/pexels-photo-2097090.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Dinner",
        url: "https://images.pexels.com/photos/2641886/pexels-photo-2641886.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Budget Meal",
        url: "https://images.pexels.com/photos/718742/pexels-photo-718742.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Budget Meal",
        url: "https://images.pexels.com/photos/70497/pexels-photo-70497.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Budget Meal",
        url: "https://images.pexels.com/photos/2725744/pexels-photo-2725744.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Breakfast",
        url: "https://images.pexels.com/photos/750073/pexels-photo-750073.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Breakfast",
        url: "https://images.pexels.com/photos/1893556/pexels-photo-1893556.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Breakfast",
        url: "https://images.pexels.com/photos/2233351/pexels-photo-2233351.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Lunch",
        url: "https://images.pexels.com/photos/1653877/pexels-photo-1653877.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Lunch",
        url: "https://images.pexels.com/photos/1199957/pexels-photo-1199957.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Lunch",
        url: "https://images.pexels.com/photos/1199958/pexels-photo-1199958.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Dinner",
        url: "https://images.pexels.com/photos/1633495/pexels-photo-1633495.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Dinner",
        url: "https://images.pexels.com/photos/1566837/pexels-photo-1566837.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Dinner",
        url: "https://images.pexels.com/photos/2619970/pexels-photo-2619970.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Budget Meal",
        url: "https://images.pexels.com/photos/1211887/pexels-photo-1211887.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Budget Meal",
        url: "https://images.pexels.com/photos/2983101/pexels-photo-2983101.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Budget Meal",
        url: "https://images.pexels.com/photos/106343/pexels-photo-106343.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Breakfast",
        url: "https://images.pexels.com/photos/1132558/pexels-photo-1132558.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Breakfast",
        url: "https://images.pexels.com/photos/1437267/pexels-photo-1437267.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Breakfast",
        url: "https://images.pexels.com/photos/1199960/pexels-photo-1199960.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Lunch",
        url: "https://images.pexels.com/photos/1860204/pexels-photo-1860204.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Lunch",
        url: "https://images.pexels.com/photos/3026808/pexels-photo-3026808.jpeg?auto=compress&cs=tinysrgb&w=600"
    },
    {
        category: "Lunch",
        url: "https://images.pexels.com/photos/1049620/pexels-photo-1049620.jpeg?auto=compress&cs=tinysrgb&w=600"
    }
];
